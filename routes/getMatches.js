...
router.get("/:firebaseUid", async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      typeof user?.preferences?.distance === "number"
    ) {
    ...
    for (let query of queriesToRun) {
      const results = await query.limit(
        targetNumberOfMatches - allMatches.length
      );
    ...
    if (matchesToSend.length === 0) {
      return res
        .status(200)
        .send({ message: "No Matches found", matches: matchesToSend });
    } else {
      return res
        .status(200)
        .send({ message: "Matches found", matches: matchesToSend });
    }
